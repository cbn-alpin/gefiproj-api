...
CREATE OR REPLACE FUNCTION affectations_sur(
    ...
    RETURNS table
            (
                id_ma             integer,          -- Identifiant du montant affecté (à NULL si la donnée vient de 'entree_sortie')
                id_r              integer,          -- Identifiant de la recette (à NULL si la donnée vient de 'entree_sortie')
                id_es             integer,          -- Identifiant de l'entrée/sortie (à NULL si la donnée vient de 'affectation')
...
BEGIN
    ...
    RETURN
        ...
        FROM montant_affecte ma
                 NATURAL JOIN recette r -- Laisser 'NATURAL JOIN' pour ne pas inclure les recettes non affectées
...
-- Utilisation, sans filtrage : SELECT * FROM affectations_sur() order by annee_recette, annee_affectation;
