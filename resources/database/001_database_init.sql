...
-- v4, modifié le 29/01/2020 : ajout de triggers
-- v3, modifié le 29/01/2020 : ajout de la table entree_sortie, des index et des index unique.
/*===========================================*/
...
DROP TABLE IF EXISTS recette_comptable CASCADE;
...
(
	id serial not null
		constraint revoked_tokens_pkey
			primary key,
	jti varchar(120)
);
...
/* Triggers */
-- Vérifie que le montant d'un financement est tjs supérieur aux montants de ses recettes, après une modification sur un financement
DROP TRIGGER IF EXISTS trigger_financement_montant
	ON financement;
CREATE OR REPLACE FUNCTION check_financement_montant()
	RETURNS trigger AS 
	$BODY$
	BEGIN
...
CREATE TRIGGER trigger_financement_montant
	BEFORE UPDATE ON financement 
	FOR EACH ROW
	EXECUTE PROCEDURE check_financement_montant();
	
-- Vérifie que le montant d'un financement est tjs supérieur aux montants de ses recettes, après un ajout/une modification d'une recette
DROP TRIGGER IF EXISTS trigger_recette_somme_montant
	ON recette;
CREATE OR REPLACE FUNCTION check_recette_somme_montant()
	RETURNS trigger AS 
	$BODY$
	BEGIN
		IF (SELECT f.montant_arrete_f FROM financement f WHERE f.id_f = NEW.id_f) < NEW.montant_r + (SELECT COALESCE(SUM(r.montant_r), 0) FROM recette r WHERE r.id_f = NEW.id_f AND r.id_r != NEW.id_r) THEN
			 RAISE EXCEPTION 'Le montant d''un financement ne peut pas être inférieur au montants de ses recettes';
		END IF;
...
CREATE TRIGGER trigger_recette_somme_montant
	BEFORE INSERT OR UPDATE ON recette
	FOR EACH ROW
	EXECUTE PROCEDURE check_recette_somme_montant();
	
-- Vérifie que le montant d'une recette est tjs supérieur aux montants de ses affectations, après une modification sur une recette
DROP TRIGGER IF EXISTS trigger_recette_montant
	ON recette;
CREATE OR REPLACE FUNCTION check_recette_montant()
	RETURNS trigger AS 
	$BODY$
	BEGIN
...
CREATE TRIGGER trigger_recette_montant
	BEFORE UPDATE ON recette 
	FOR EACH ROW
	EXECUTE PROCEDURE check_recette_montant();
	
-- Vérifie que le montant d'une recette est tjs supérieur aux montants de ses affectations, après un ajout/une modification sur les affectations
DROP TRIGGER IF EXISTS trigger_montantaffecte_montant
	ON montant_affecte;
CREATE OR REPLACE FUNCTION check_montantaffecte_montant()
	RETURNS trigger AS 
	$BODY$
	BEGIN
		IF (SELECT r.montant_r FROM recette r WHERE r.id_r = NEW.id_r) < NEW.montant_ma + (SELECT COALESCE(SUM(ma.montant_ma), 0) FROM montant_affecte ma WHERE ma.id_r = NEW.id_r AND ma.id_ma != NEW.id_ma) THEN
			 RAISE EXCEPTION 'Le montant d''une recette ne peut pas être inférieur au montants de ses affectations';
		END IF;
...
CREATE TRIGGER trigger_montantaffecte_montant
	BEFORE INSERT OR UPDATE ON montant_affecte
	FOR EACH ROW
	EXECUTE PROCEDURE check_montantaffecte_montant();
